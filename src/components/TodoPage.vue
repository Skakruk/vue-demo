...
<script>
  ...
  // const API_ENDPOINT = 'http://localhost:8888/todo';
  ...
  export default {
    ...
    created() {
      axios.get(API_ENDPOINT, { withCredentials: true })
        .then((response) => {
    ...
    methods: {
      ...
      deleteItem(deleteItemId) {
        axios.delete(`${API_ENDPOINT}/${deleteItemId}`, { withCredentials: true })
          .then(() => {
      ...
      deleteAllCompleted() {
        const requests = this.items.filter(item => item.isCompleted)
          .map(item => axios.delete(`${API_ENDPOINT}/${item.id}`, { withCredentials: true }));
      ...
      toggleAll(markCompleted) {
        const requests = this.items.map(item => axios.put(`${API_ENDPOINT}/${item.id}`, {
